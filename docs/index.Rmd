...
# Eventos futuros e recentes no Brasil

Dados de eventos recentes obtidos no [calendário do R Community Explorer](https://r-community.github.io/event-explorer/rcalendar.html) 
...
eventos_recentes_brasil %>% 
  ...
  dplyr::slice(1:5) %>% 
   ...
   dplyr::mutate(title = stringr::str_remove_all(title, c(", Brazil, Latin America")),
                 title_link = glue::glue("[{title}]({url})"),
                 data = lubridate::as_date(start),
                 data = format(data, "%d/%b/%Y", locale = "pt_BR")) %>%
   dplyr::select(group, title_link, data) %>%
   knitr::kable(col.names = c("Capítulo", "Nome do evento", "Data"))



```

Thanks to [Ben Ubah](https://github.com/benubah) for [this suggestion](https://github.com/rladies/meetupr/issues/79#issuecomment-743910328)!
---
class: 
